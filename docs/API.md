...
Публичный HTTP API для рендера Hearthstone-колод, распознавания архетипов и перевода названий. Документ сохраняет англоязычные примеры запросов, чтобы ими было удобно пользоваться из кода.
